...
function toApiPayload(result: StaffReentryResult) {
  return {
...
/**
 * Blueprint 7.3X — thin authenticated staff re-entry API.
 * POST /api/events/{eventId}/re-entries
 *
 * Delegates entirely to 7.3W `staffReenterRegisteredAttendee`.
...
export async function POST(request: Request, context: RouteContext) {
  ...
  try {
    ...
    return NextResponse.json(
      ...
      { status: result.outcome === "ALREADY_PRESENT" ? 200 : 201 },
    );
  } catch (error) {
