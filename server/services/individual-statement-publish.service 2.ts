...
export class IndividualStatementPublishError extends Error {
  constructor(
    ...
    super(message);
    this.name = "IndividualStatementPublishError";
  }
}
 ...
 * INDIVIDUAL statement from GENERATED to PUBLISHED. Email and household
 * publishing are later patches.
 */
export async function publishIndividualContributionStatement(input: {
  statementId: string;
}) {
  ...
  if (!actor) {
    throw new IndividualStatementPublishError(
      "SIGNED_OUT",
  ...
  if (!statementIdSchema.safeParse(input.statementId).success) {
    throw new IndividualStatementPublishError("NOT_FOUND", "Statement not found.");
  }
